...
namespace App1
{
    /*=======================================================================
        ...
        the various prevention methods for Cholera. This information is pulled 
        from the DB.

    ========================================================================*/
    [Activity(Label = "Prevention Methods")]
    public class PreventionMethodsActivity : Activity
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            SetContentView(Resource.Layout.PreventionMethodsLayout);
        ...
        ***************************************************************************/
        async Task populateInfo()
        {
            ...
            LinearLayout linearLayout = FindViewById<LinearLayout>(Resource.Id.preventionsLinearLayout);
        ...
        ***************************************************************************/
        public async Task callAPIEndpoint()
        {
            try
            {
                ...
                request.RequestUri = new Uri("http://choltechcloudservice.cloudapp.net/api/CholeraController/getAllPreventionMethods");
                ...
                if (response.StatusCode == System.Net.HttpStatusCode.OK)
                {
                    ...
                    if (content != null)
                    {
                        ...
                        {
                            Toast.MakeText(Application.Context, "Prevention Methods Fetched!", ToastLength.Short).Show();
                            Console.WriteLine("DATA ------------{0}", JsonConvert.DeserializeObject(jToken["data"].ToString()));

                            foreach (var i in jToken["data"])
                            {
                                Console.WriteLine("Prevention ------------{0}", i);
                            }
                        ...
                        {
                            Toast.MakeText(Application.Context, "Error Fetching Prevention Methods!" + jToken["message"].ToString(), ToastLength.Short).Show();
                        }
                    }
                    else
                    {
                        Toast.MakeText(Application.Context, "Error Fetching Prevention Methods!", ToastLength.Short).Show();
                    }
                }
                else
                {
                    Toast.MakeText(Application.Context, "Error Fetching Prevention Methods! Code: " + response.StatusCode, ToastLength.Short).Show();
                }
