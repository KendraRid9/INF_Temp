...
namespace App1
{
    [Activity(Label = "Risk Factors")]
    public class RiskFactorsActivity : Activity
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            SetContentView(Resource.Layout.RiskFactorsLayout);
        ...
        ***************************************************************************/
        async Task populateInfo()
        {
            ...
            LinearLayout linearLayout = FindViewById<LinearLayout>(Resource.Id.riskFactorsLinearLayout);
        ...
        ***************************************************************************/
        public async Task callAPIEndpoint()
        {
            try
            {
                ...
                request.RequestUri = new Uri("http://choltechcloudservice.cloudapp.net/api/CholeraController/getAllRiskFactors");
                ...
                if (response.StatusCode == System.Net.HttpStatusCode.OK)
                {
                    ...
                    if (content != null)
                    {
                        ...
                        {
                            Toast.MakeText(Application.Context, "Risk-Factors Fetched!", ToastLength.Short).Show();
                            Console.WriteLine("DATA ------------{0}", JsonConvert.DeserializeObject(jToken["data"].ToString()));

                            foreach (var i in jToken["data"])
                            {
                                Console.WriteLine("RISK ------------{0}", i);
                            }
                        ...
                        {
                            Toast.MakeText(Application.Context, "Error Fetching Risk-Factors!" + jToken["message"].ToString(), ToastLength.Short).Show();
                        }
                    }
                    else
                    {
                        Toast.MakeText(Application.Context, "Error Fetching Risk-Factors!", ToastLength.Short).Show();
                    }
                }
                else
                {
                    Toast.MakeText(Application.Context, "Error Fetching Risk-Factors! Code: " + response.StatusCode, ToastLength.Short).Show();
                }
