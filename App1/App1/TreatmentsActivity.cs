...
namespace App1
{
    [Activity(Label = "Treatments")]
    public class TreatmentsActivity : Activity
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            SetContentView(Resource.Layout.TreatmentsLayout);
        ...
        ***************************************************************************/
        public async Task callAPIEndpoint()
        {
            try
            {
                ...
                request.RequestUri = new Uri("http://choltechcloudservice.cloudapp.net/api/CholeraController/getAllTreatments");
                ...
                if (response.StatusCode == System.Net.HttpStatusCode.OK)
                {
                    ...
                    if (content != null)
                    {
                        ...
                        {
                            Toast.MakeText(Application.Context, "Error Fetching Treatments!" + jToken["message"].ToString(), ToastLength.Short).Show();
                        }
                    }
                    else
                    {
                        Toast.MakeText(Application.Context, "Error Fetching Treatments!", ToastLength.Short).Show();
                    }
                }
                else
                {
                    Toast.MakeText(Application.Context, "Error Fetching Treatments! Code: " + response.StatusCode, ToastLength.Short).Show();
                }
